          ...
          tryCallback: async () => {
            ...
            for (const jsFile of jsFiles) {
              ...
              const mergedContent = await fs.readFile(
                path.join(mergeDir, jsFile),
                "utf-8",
              );
              const aiGen = handleAiGeneration(
                appId!,
                dir,
                jsFile,
                mergedContent,
...
const handleAiGeneration = async function* (
  ...
  const hasAiTemplates = promptTemplates.some(
    ({ name }) => !name.startsWith("00_"),
  );
  ...
  if (hasAiTemplates) {
    ...
    if (
      firstResponse.startsWith("AI APIサーバーに接続できませんでした。") ||
      firstResponse.startsWith("AI APIの呼び出しに失敗しました")
    ) {
      console.error(
        `  [Error] AIの初期解析に失敗したため、アプリID: ${appId} のAI処理を中止します。`,
      );
      await writeErrorLog(
        appDir,
        `AIの初期解析中にエラーが発生しました。`,
  ...
  await matches.reduce(async (promise, { functionalName, lineNumber }) => {
    ...
    const fullPrompt = promptTemplates
      .map((t) =>
        t.content
          .split("{{fileName}}")
          .join(outputFileName)
    ...
    if (cached) {
      await Promise.all([
        ...
        fs.writeFile(
          path.join(promptsDir, resultFileName),
          fullPrompt,
          "utf-8",
        ),
      ]);
      ...
      if (parts.length === promptTemplates.length) {
        await Promise.all(
          promptTemplates.map((pt, i) => {
            ...
            return fs.writeFile(
              path.join(resultsDir, individualResultFileName),
              parts[i],
              "utf-8",
            );
          }),
    ...
    for (let i = 0; i < promptTemplates.length; i++) {
      ...
      const prompt = content
        ...
        .join(functionalName)
        .split("{{content}}")
        .join("提示済みのコードを参照してください。");

      aiMessages.push({ role: "user", content: prompt });
      console.log(
        `  [AI] ${functionalName} の回答を生成中 (${i + 1}/${promptTemplates.length})...`,
      );
      ...
      if (
        res.startsWith("AI APIサーバーに接続できませんでした。") ||
        res.startsWith("AI APIの呼び出しに失敗しました")
      ) {
        console.error(
          `  [Error] ${functionalName} の回答生成に失敗しました (${name})。このマーカーの処理を中断し、ファイルは保存しません。`,
        );
        await writeErrorLog(
          appDir,
          `${functionalName} の回答生成中にAIエラーが発生しました (${name})`,
          res,
        );
    ...
    if (!markerFailed && markerResults.length > 0) {
      ...
      console.log(
        `  [OK] AIの結果を保存しました: prompts_results/${resultFileName}`,
      );
    }
  }, Promise.resolve());
